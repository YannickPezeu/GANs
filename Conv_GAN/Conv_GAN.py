...
optimizerG = optim.Adam(list(generator.parameters()))
...
def train_discriminator(n_batchs):
    ...
    for i, data in enumerate(trainloader, 0):
        ...
        fake_truth_prediction_loss = truth_criterion(fake_truth_prediction, torch.zeros_like((fake_truth_prediction)))

        full_lossD = \
            class_prediction_loss + \
            real_truth_prediction_loss + \
            fake_truth_prediction_loss
...
def train_generator(n_batchs):
    ...
    for i in range(n_batchs):
        ...
        full_loss_gen = truth_lossG + class_lossG
...
if __name__ == '__main__':
    ...
    SEED = generate_noise_vector([0]*25)
    def generate_fake_image_grid(path):

            fake_images = generator(SEED).reshape(-1,1, 28,28).cpu().detach()
    ...
    def full_train2(n_cycle, length_cycle_gen, length_cycle_disc):
        ...
        for i in range(n_cycle):
            ...
            if i % 10 == 9:
                print( i, "gen_loss:", gen_loss, "disc_loss:", disc_loss)
            if i % 50 == 49:
                print("save")
                generate_fake_image_grid(SAVING_DIR + "image_grid" + str(72 + i//50))
                torch.save(discriminator.state_dict(),SAVING_DIR + 'discriminator'+"_gen"+str(length_cycle_gen) + "_disc" + str(length_cycle_disc))
                torch.save(generator.state_dict(), SAVING_DIR + 'generator' + "_gen"+str(length_cycle_gen) + "_disc" + str(length_cycle_disc))
    ...
    def load_models():
        try:
            discriminator.load_state_dict(torch.load(SAVING_DIR + 'discriminator'+"_gen"+str(LENGTH_CYCLE_GEN) + "_disc" + str(LENGTH_CYCLE_DISC)))
            generator.load_state_dict(torch.load( SAVING_DIR + 'generator'+"_gen"+str(LENGTH_CYCLE_GEN) + "_disc" + str(LENGTH_CYCLE_DISC)))
            print("loaded models")
        except:
            print("couldn't load models, training from scratch")

    load_models()
    full_train2(10000, length_cycle_gen = LENGTH_CYCLE_GEN, length_cycle_disc= LENGTH_CYCLE_DISC)
    # train_disc_only(20)
    # train_gen_only(20)
